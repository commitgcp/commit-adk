...
from agents.notion.agent import create_agent as create_notion_adk_agent # Renamed for clarity
...
class NotionA2AWrapper:
    ...
    async def create(cls):
        ...
        wrapper = cls()
        wrapper._underlying_agent, wrapper._exit_stack = await create_notion_adk_agent()
        wrapper._runner = Runner(
            session_service=session_service,
            agent=wrapper._underlying_agent,
            app_name="notion_agent", # Consistent with original
        )
        return wrapper
    ...
    async def close(self):
        ...
        if self._exit_stack:
            ...
            self._exit_stack = None
            print("NotionA2AWrapper: MCP exit_stack closed.")

    def get_processing_message(self) -> str:
        return "The Notion agent is thinking..."
